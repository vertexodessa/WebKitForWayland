...
void PeerConnectionBackendQt5WebRTC::setConfiguration(RTCConfiguration& config, const MediaConstraints& constraints)
{
    ...
    // FIXME: naming
    for (auto& c : mediaConstraints.keys()) {
        std::string name = c.utf8().data();
        MediaConstraintType type(mediaConstraints.get(c)->type());
    ...
    Vector<MediaTrackConstraintSetMap> mediaConstraints_ = constraints.advancedConstraints();
    mediaConstraints_ = constraints.advancedConstraints();
    for (const auto& ctrns : mediaConstraints_) {
        for (auto& c : ctrns.keys()) {
            std::string name = c.utf8().data();
            MediaConstraintType type(mediaConstraints.get(c)->type());
            std::string value = RealtimeMediaSourceSupportedConstraints::nameForConstraint(type).string().utf8().data();
            wrtcConstraints[name] = value;
